# https://github.com/thegraydream/TikTok-TTS
# # ╭─────────────────────────── Import ───────────────────────────╮ #
import os
import json
import base64
...
def texttotiktoktts(text, voice="en_us_001", path="", file_name="tts_audio.mp3"):
     ...
     try:
          ...
          if error == None:
               ...
               with open(f'{path}/{file_name}.mp3', "wb") as file:
                    file.write(audio_data) # Write audio in mp3
               if log_statut == True:print(f'{reset}[{green}+{reset}] {green}the "{text}" audio file has been exported to "{path}\\{file_name}".{reset}') # Log Print
               return True, (f"{path}\\{file_name}.mp3")
          else:
